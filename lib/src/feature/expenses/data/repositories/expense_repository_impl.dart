...
final class ExpenseRepositoryImpl implements ExpenseRepository {
  ...
  EitherFuture<ExpenseModel> createExpense(ExpenseModel expense) async {
    try {
      if (!await _networkInfo.isConnected) {
        final result = await _localDataSource.addExpense(expense as ExpenseDo);
        return Right(result as ExpenseModel);
      }

      final result = await _remoteDataSource.createExpense(expense);
      await _localDataSource.addExpense(result as ExpenseDo);
      return Right(result);
    } catch (e) {
  ...
  Future<void> syncExpenses() async {
    if (await _networkInfo.isConnected) {
      ...
      await _localDataSource.saveExpenses(remoteExpenses as List<ExpenseDo>);
    }
  }
}
